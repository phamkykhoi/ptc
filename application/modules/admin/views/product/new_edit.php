<form method="post" action="" name="account_form" enctype="multipart/form-data"> 
    <div class="group">
        ...
        <div class="infobasic">
            ...
            <div>
                <textarea name="video" style="width: 605px; height: 80px;"><?php echo htmlentities($product['video_youtube']); ?></textarea>
            </div>
        </div>
        ...
        <p style="font-size:14px; color:#F00"><b>Chú ý:</b>Chấp nhận các file ảnh: gif, jpg, png<br />
            Nên để kích thước ảnh 640 x 640 thì chất lượng ảnh sản phẩm sẽ đẹp </p>
        <p>
            <?php
        ...
        <?php 
            if(isset($thumbs) && $thumbs != NULL){
                foreach($thumbs as $value){
                    echo "<img style='max-width:100px;max-height:70px;margin-right:5px;' src='".base_url()."uploads/images_products/thumbs/".$value['url']."' />
                          <a href='".base_url('admin/product/deleteimage/'. $value['id']. '/'. $value['product_id'])."'>Xóa ảnh</a>&nbsp;&nbsp;";
                }
            }
        ?>
